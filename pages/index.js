...
const workoutsData = {
  Monday: [
    ...
    // ... same as before for all days
  ],
  // Add other days as before, omitted for brevity; use previous full data
};
...
export default function Home() {
  ...
  // Sync dark mode class on body for global styles
  useEffect(() => {
    if (typeof window !== "undefined") {
    ...
    return (
      <div className={`card ${markedDone ? "done" : ""}`}>
        ...
        {interactive && (
          <button
            disabled={
              !markedDone && (!selectedSets || (!selectedReps && selectedReps !== manualEntryLabel))
            }
  ...
  return (
    <>
      <style jsx global>{`
        ...
        /* Reset bottom fixed nav height for consistent padding */
        main {
          max-width: 480px;
          margin: 0 auto;
